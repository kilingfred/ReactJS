...
export default function UpperCanvas({ difficulty = "easy" }: UpperCanvasProps) {
  ...
  const { setOperators, setNumbers, getResult, numbers, operators } =
    useAppContext().resultValue; // Access ResultValue context
  ...
  const generateEquationJSX = (
    numbersDisplay: React.ReactNode[],
    operators: string[],
    answer: number | null
  ) => (
    <div
  ...
  return (
    <>
      <div className="upperCanvas">
        ...
        <div className="equation">{equationJSX}</div>
      </div>
      {numbers.length === 2 &&
        operators.length === 1 &&
        !isNaN(getResult()) && (
